...
////////////////////////////////////////////////////////////////////////////////
NSString * IGolfplayerErrorDomain = @"ru.kolyvan.kxmovie";
static void FFLog(void* context, int level, const char* format, va_list args);

static NSError * IGplayerError (NSInteger code, id info)
{
    ...
    return [NSError errorWithDomain:IGolfplayerErrorDomain
                               code:code
                           userInfo:userInfo];
}

static NSString * errorMessage (IGolfplayerError errorCode)
{
    switch (errorCode) {
        case IGolfplayerErrorNone:
            return @"";
            
        case IGolfplayerErrorOpenFile:
            return NSLocalizedString(@"Unable to open file", nil);
            
        case IGolfplayerErrorStreamInfoNotFound:
            return NSLocalizedString(@"Unable to find stream information", nil);
            
        case IGolfplayerErrorStreamNotFound:
            return NSLocalizedString(@"Unable to find stream", nil);
            
        case IGolfplayerErrorCodecNotFound:
            return NSLocalizedString(@"Unable to find codec", nil);
            
        case IGolfplayerErrorOpenCodec:
            return NSLocalizedString(@"Unable to open codec", nil);
            
        case IGolfplayerErrorAllocateFrame:
            return NSLocalizedString(@"Unable to allocate frame", nil);
            
        case IGolfplayerErrorSetupScaler:
            return NSLocalizedString(@"Unable to setup scaler", nil);
            
        case IGolfplayerErroReSampler:
            return NSLocalizedString(@"Unable to setup resampler", nil);
            
        case IGolfplayerErrorUnsupported:
...
@interface IGMovieFrame()
@property (readwrite, nonatomic) CGFloat position;
@property (readwrite, nonatomic) CGFloat duration;
@end

@implementation IGMovieFrame
@end

@interface IGAudioFrame()
@property (readwrite, nonatomic, strong) NSData *samples;
@end

@implementation IGAudioFrame
- (IGFrameType) type { return IGFrameTypeAudio; }
@end

@interface IGVideoFrame()
@property (readwrite, nonatomic) NSUInteger width;
@property (readwrite, nonatomic) NSUInteger height;
@end

@implementation IGVideoFrame
- (IGFrameType) type { return IGFrameTypeVideo; }
@end

@interface IGVideoFrameRGB ()
@property (readwrite, nonatomic) NSUInteger linesize;
@property (readwrite, nonatomic, strong) NSData *rgb;
@end

@implementation IGVideoFrameRGB
- (IGVideoFrameFormat) format { return IGVideoFrameFormatRGB; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@implementation IGVideoFrameYUV
- (IGVideoFrameFormat) format { return IGVideoFrameFormatYUV; }
@end

@interface IGArtworkFrame()
@property (readwrite, nonatomic, strong) NSData *picture;
@end

@implementation IGArtworkFrame
- (IGFrameType) type { return IGFrameTypeArtwork; }
- (UIImage *) asImage
{
    UIImage *image = nil;
...
@interface IGSubtitleFrame()
@property (readwrite, nonatomic, strong) NSString *text;
@end

@implementation IGSubtitleFrame
- (IGFrameType) type { return IGFrameTypeSubtitle; }
@end

////////////////////////////////////////////////////////////////////////////////

@interface IGolfplayerDecoder () {
    ...
    NSDictionary        *_info;
    IGVideoFrameFormat  _videoFrameFormat;
    NSUInteger          _artworkStream;
    NSInteger           _subtitleASSEvents;
}
@end

@implementation IGolfplayerDecoder
...
- (void) setPosition: (CGFloat)seconds
{
    ...
    if (_videoStream != -1) {
        ...
        avformat_seek_file(_formatCtx, _videoStream, ts, ts, ts, AVSEEK_FLAG_ANY);//AVSEEK_FLAG_FRAME);
        avcodec_flush_buffers(_videoCodecCtx);
    }
    
    if (_audioStream != -1) {
        int64_t ts = (int64_t)(seconds / _audioTimeBase);
        avformat_seek_file(_formatCtx, _audioStream, ts, ts, ts, AVSEEK_FLAG_ANY);//AVSEEK_FLAG_FRAME);
...
- (void) setSelectedAudioStream:(NSInteger)selectedAudioStream
{
    ...
    IGolfplayerError errCode = [self openAudioStream: audioStream];
    if (IGolfplayerErrorNone != errCode) {
...
- (void) setSelectedSubtitleStream:(NSInteger)selected
{
    ...
    if (selected == -1) {
        ...
        IGolfplayerError errCode = [self openSubtitleStream:subtitleStream];
        if (IGolfplayerErrorNone != errCode) {
...
{
    IGolfplayerDecoder *mp = [[IGolfplayerDecoder alloc] init];
    if (mp) {
            ...
            error: (NSError **) perror
{
    ...
    IGolfplayerError errCode = [self openInput: path];
    
    if (errCode == IGolfplayerErrorNone) {
        
        IGolfplayerError videoErr = [self openVideoStream];
        IGolfplayerError audioErr = [self openAudioStream];
        
        _subtitleStream = -1;
        
        if (videoErr != IGolfplayerErrorNone &&
            audioErr != IGolfplayerErrorNone) {
    ...
    if (errCode != IGolfplayerErrorNone) {
        ...
        if (perror)
            *perror = IGplayerError(errCode, errMsg);
        return NO;
    }    
    _isEOF = NO;
...
- (IGolfplayerError) openInput: (NSString *) path
{
    ...
    if (_interruptCallback) {
        ...
        if (!formatCtx)
            return IGolfplayerErrorOpenFile;
    ...
    if (avformat_find_stream_info(formatCtx, NULL) < 0) {
        ...
        return IGolfplayerErrorStreamInfoNotFound;
    }

    av_dump_format(formatCtx, 0, [path.lastPathComponent cStringUsingEncoding: NSUTF8StringEncoding], false);
    
    _formatCtx = formatCtx;
    return IGolfplayerErrorNone;
}

- (IGolfplayerError) openVideoStream
{
    IGolfplayerError errCode = IGolfplayerErrorStreamNotFound;
    _videoStream = -1;
    _artworkStream = -1;
...
- (IGolfplayerError) openVideoStream: (NSInteger) videoStream
{    
    ...
    if (!codec)
        return IGolfplayerErrorCodecNotFound;
    ...
    if (avcodec_open2(codecCtx, codec, NULL) < 0)
        return IGolfplayerErrorOpenCodec;
        
    _videoFrame = av_frame_alloc();

    if (!_videoFrame) {
        avcodec_close(codecCtx);
        return IGolfplayerErrorAllocateFrame;
    }
...
- (IGolfplayerError) openAudioStream
{
    IGolfplayerError errCode = IGolfplayerErrorStreamNotFound;
    _audioStream = -1;
...
- (IGolfplayerError) openAudioStream: (NSInteger) audioStream
{   
    ...
    if (!audioCodecIsSupported(codecCtx)) {
        ...
        if (!swrContext ||
            ...
            return IGolfplayerErroReSampler;
        }
    }
...
- (IGolfplayerError) openSubtitleStream: (NSInteger) subtitleStream
{
    ...
    if (codecDesc && (codecDesc->props & AV_CODEC_PROP_BITMAP_SUB)) {
        ...
        return IGolfplayerErrorUnsupported;
    }
    ...
    if (codecCtx->subtitle_header_size) {
        ...
        if (s.length) {
            ...
            NSArray *fields = [IGolfplayerSubtitleASSParser parseEvents:s];
...
- (IGVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == IGVideoFrameFormatYUV) {
            
        IGVideoFrameYUV * yuvFrame = [[IGVideoFrameYUV alloc] init];
        ...
        IGVideoFrameRGB *rgbFrame = [[IGVideoFrameRGB alloc] init];
...
- (IGAudioFrame *) handleAudioFrame
{
    ...
    IGAudioFrame *frame = [[IGAudioFrame alloc] init];
...
- (IGSubtitleFrame *) handleSubtitle: (AVSubtitle *)pSubtitle
{
    NSMutableString *ms = [NSMutableString string];
        ...
        if (rect) {
            ...
            if (rect->text) { // rect->type == SUBTITLE_TEXT
                ...
                if (s.length) {
                    ...
                    NSArray *fields = [IGolfplayerSubtitleASSParser parseDialogue:s numFields:_subtitleASSEvents];
                    if (fields.count && [fields.lastObject length]) {
                        
                        s = [IGolfplayerSubtitleASSParser removeCommandsFromEventText: fields.lastObject];
                        if (s.length) [ms appendString:s];
                    }                    
                }
            }
    ...
    IGSubtitleFrame *frame = [[IGSubtitleFrame alloc] init];
    frame.text = [ms copy];   
...
{
    if (format == IGVideoFrameFormatYUV &&
        _videoCodecCtx &&
        (_videoCodecCtx->pix_fmt == AV_PIX_FMT_YUV420P || _videoCodecCtx->pix_fmt == AV_PIX_FMT_YUVJ420P)) {
        
        _videoFrameFormat = IGVideoFrameFormatYUV;
        return YES;
    }
    
    _videoFrameFormat = IGVideoFrameFormatRGB;
        ...
        if (packet.stream_index ==_videoStream) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    ...
                    IGVideoFrame *frame = [self handleVideoFrame];
                    if (frame) {
            ...
            while (pktSize > 0) {
                ...
                if (gotframe) {
                    IGAudioFrame * frame = [self handleAudioFrame];
                    if (frame) {
            ...
            if (packet.size) {
                ...
                IGArtworkFrame *frame = [[IGArtworkFrame alloc] init];
            ...
            while (pktSize > 0) {
                ...
                if (gotsubtitle) {
                    ...
                    IGSubtitleFrame *frame = [self handleSubtitle: &subtitle];
                    if (frame) {
                        [result addObject:frame];
                    }
...
static int interrupt_callback(void *ctx)
{
    if (!ctx)
        return 0;
    __unsafe_unretained IGolfplayerDecoder *p = (__bridge IGolfplayerDecoder *)ctx;
...
@implementation IGolfplayerSubtitleASSParser
